...
export default function CreateListing() {
  const { curlouerUser } = useSelector((state) => state.user);
  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError("Échec du téléchargement de l'image (2 Mo maximum par image)");
          setUploading(false);
        });
    } else {
      setImageUploadError('Vous ne pouvez télécharger que 6 images par annonce');
      setUploading(false);
    }
  };
  ...
  const handleChange = (e) => {
    if (e.target.id === 'vente' || e.target.id === 'louer') {
      setFormData({
        ...formData,
        type: e.target.id,
      });
